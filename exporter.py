...
def save_raw_keyframe_dic(f, raw_keyframe_dic):
    for (key, value) in raw_keyframe_dic.items():
        for (key2, value2) in enumerate(value):
            item_list = ["RawKeyframe", str(key), str(key2), str(len(value))]
            item_list.extend([str(item) for item in value2])
            f.write("[{}]\r\n".format(",".join(item_list)))
        f.write("\r\n")
...
def make_hierarchy_dic(hierarchy_dic, geometry_node_mesh_dic, use_only_deform_bones, use_rigify_armature, use_rigify_root_bone, use_only_selected_deform_bones, block_list, context_objects):
    for (index, ob) in enumerate(context_objects):
        ...
        elif ob.type in ['CURVE', 'SURFACE', 'FONT', 'META']:
            depsgraph = bpy.context.evaluated_depsgraph_get()
            ob_eval = ob.evaluated_get(depsgraph)
            tmp_me = bpy.data.meshes.new_from_object(ob_eval, preserve_all_data_layers=True, depsgraph=depsgraph)
            if len(tmp_me.polygons) == 0:
                bpy.data.meshes.remove(tmp_me)
            else:
                tmp_ob = bpy.data.objects.new(ob.name, tmp_me)
                tmp_ob.matrix_world = ob.matrix_world
                tmp_ob.parent = ob.parent
                hierarchy_dic[str(index)] = ['MESH', ob.name, str(-1), None, tmp_ob]
                geometry_node_mesh_dic[tmp_ob] = (ob, tmp_me)
    ...
    for (key, value) in hierarchy_dic.items():
        ...
        while parent_node != None:
            for (key2, value2) in hierarchy_dic.items():
                node2 = value2[-1] if value2[-1] not in geometry_node_mesh_dic else geometry_node_mesh_dic[value2[-1]][0]
...
def guess_texture_type(material, texture_file_name):
    texture_type = -1
    if bpy.context.scene.render.engine == 'CYCLES' or bpy.context.scene.render.engine == 'BLENDER_EEVEE' or bpy.context.scene.render.engine == 'BLENDER_EEVEE_NEXT':
        # material use node
                        ...
                        if material.node_tree != None:
                            for node in material.node_tree.nodes:
                                if node.bl_idname == 'ShaderNodeEmission':
                                    ...
                                    diffuse_color[1] = 0.0
                                    diffuse_color[2] = 0.0
                                    emissive_color[0] = node.inputs['Color'].default_value[0]
                                    emissive_color[1] = node.inputs['Color'].default_value[1]
                                    emissive_color[2] = node.inputs['Color'].default_value[2]
                                    emissive_strength = node.inputs['Strength'].default_value
                                ...
                                if node.bl_idname == 'ShaderNodeBsdfPrincipled':
                                    ...
                                    emissive_strength = node.inputs['Emission Strength'].default_value
                                ...
                                if node.bl_idname == 'ShaderNodeTexImage':
                                    if node.image != None and node.image.source != 'GENERATED':
                                        (_, ext) = os.path.splitext(node.image.filepath_from_user())
                                        # fix empty filepath for packed file
                                        if ext == '' and node.image.packed_file != None:
                                            ext = node.image.file_format.lower()
                                            node.image.filepath_raw = "{}.{}".format(node.image.name, ext)
                                        if ext != '':
...
def make_generic_node_dic(context, hierarchy_dic, use_only_deform_bones, node_dic, default_pose_dic, bind_pose_dic, vertex_dic, weight_dic, shape_dic, polygon_dic, uv_dic, tangent_dic, normal_dic, color_dic, polygon_material_dic, texture_dic, material_dic, mesh_material_dic, exist_object_dic, use_animation, use_timeline_range, my_animation_offset, my_animation_type, block_list, use_rigify_armature, camera_dic, light_dic, custom_property_dic, edge_crease_dic, use_edge_crease, my_edge_crease_scale, my_edge_smoothing, edge_smoothing_dic, applied_mesh_dic, ik_dic, use_embed_media, use_copy_texture, subdirname, packed_texture_filenames, bone_correction_matrix, use_tangents, use_export_materials):
    ...
    for (key, value) in hierarchy_dic.items():
        ...
        if value[0] == 'MESH':
            ...
            # Note: Sometimes it will break normals after validation, so I commented this line.
...
def get_token_from_data_path(data_path):
    tokens = data_path.split('.')
    return tokens[-1]


def make_pose_key_dic(context, node_dic, key, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, ob_parent, ob, frame_count, frame, frame_index, my_animation_type, my_animation_offset, is_bone, bone_correction_matrix):
    if frame == 0:
                    ...
                    for fcurve in ob.animation_data.action.fcurves:
                        for keyframe in fcurve.keyframe_points:
                            if not fcurve.data_path.startswith('pose.bones'):
                                temp_list.append((get_token_from_data_path(fcurve.data_path), fcurve.array_index, int(keyframe.co[0]) + my_animation_offset, keyframe.interpolation))
                    if len(temp_list) > 0:
                        index2 = len(raw_keyframe_dic)
                        raw_keyframe_dic[index2] = temp_list
                        make_node(node_dic, key, 'RawKeyframe', '', index2)
            else:
                if ob_parent.animation_data.action != None:
                    temp_list = []
                    for fcurve in ob_parent.animation_data.action.fcurves:
                        for keyframe in fcurve.keyframe_points:
...
def make_pose_key_node_dic(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, my_animation_type, use_timeline_range, my_animation_offset, desired_object, bone_correction_matrix):
    (action_start, action_end) = get_pose_key_range(context, hierarchy_dic, my_animation_type)
    if use_timeline_range:
        (action_start, action_end) = (max(action_start, context.scene.frame_start), min(action_end, context.scene.frame_end))
    # exists any pose key
        ...
        for frame in range(action_start, action_end + 1):
            ...
            for (key, value) in hierarchy_dic.items():
                ...
                if ob_parent == None:
                    ...
                    if ob.animation_data != None and ((ob.animation_data.use_nla and len(ob.animation_data.nla_tracks) > 0) or ob.animation_data.action != None):
                        make_pose_key_dic(context, node_dic, key, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, ob_parent, ob, action_end + 1 - action_start, frame - action_start, frame, my_animation_type, my_animation_offset, node_type == 'BONE', bone_correction_matrix)
                else:
                    # object has animation
...
def remove_geometry_node_mesh_dic(geometry_node_mesh_dic):
    # remove geometry node meshes from data pool
    for geometry_node_mesh in geometry_node_mesh_dic.values():
        bpy.data.meshes.remove(geometry_node_mesh[1])
...
def make_all_actions(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic):
    for (key, value) in hierarchy_dic.items():
        ob = value[-1]
        for action in bpy.data.actions:
            ...
            if value[0] != 'BONE' and ob.animation_data != None:
                ...
                if action.id_root == 'OBJECT':
                    ...
                    make_pose_key_node_dic(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, my_animation_type, use_timeline_range, my_animation_offset, None, bone_correction_matrix)
...
def make_all_nla_tracks(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic):
    for (key, value) in hierarchy_dic.items():
        ob = value[-1]
        if value[0] != 'BONE' and ob.animation_data != None:
            if ob.animation_data.use_nla and len(ob.animation_data.nla_tracks) > 0:
                ...
                for i in range(len(ob.animation_data.nla_tracks)):
                    ...
                    make_pose_key_node_dic(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, my_animation_type, use_timeline_range, my_animation_offset, ob, bone_correction_matrix)
                    if use_driver_based_shape_key:
                        if ob.type == 'ARMATURE':
                            for (key2, value2) in hierarchy_dic.items():
                                ob2 = value2[-1]
...
def make_active_action_or_nla_tracks(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic):
    # transform actions
    make_pose_key_node_dic(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, my_animation_type, use_timeline_range, my_animation_offset, None, bone_correction_matrix)
    for (key, value) in hierarchy_dic.items():
        ob = value[-1]
...
def write_some_data(context, filepath, context_objects, use_animation, use_timeline_range, my_animation_offset, use_driver_based_shape_key, my_animation_type, use_only_deform_bones, use_rigify_armature, use_rigify_root_bone, use_only_selected_deform_bones, my_max_bone_influences, primary_bone_axis, secondary_bone_axis, use_vertex_animation, use_vertex_format, use_vertex_space, my_vertex_frame_start, my_vertex_frame_end, use_edge_crease, my_edge_crease_scale, my_edge_smoothing, use_apply_modifiers, use_include_armature_deform_modifier, use_concatenate_all, use_embed_media, use_copy_texture, subdirname, packed_texture_filenames, use_tangents, use_export_materials, use_raw_keyframe):
    ...
    make_hierarchy_dic(hierarchy_dic, geometry_node_mesh_dic, use_only_deform_bones, use_rigify_armature, use_rigify_root_bone, use_only_selected_deform_bones, block_list, context_objects)
    ...
    if use_animation:
        if use_vertex_animation:
            ...
            if my_animation_type == 'Active':
                make_active_action_or_nla_tracks(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic)
            elif my_animation_type == 'Actions':
                make_all_actions(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic)
            elif my_animation_type == 'Tracks':
                make_all_nla_tracks(context, hierarchy_dic, node_dic, pose_key_dic, use_raw_keyframe, raw_keyframe_dic, shape_key_dic, camera_key_dic, use_driver_based_shape_key, my_animation_type, use_timeline_range, my_animation_offset, bone_correction_matrix, exist_object_dic)
    ...
    # remove any geometry node meshes
    remove_geometry_node_mesh_dic(geometry_node_mesh_dic)
    ...
    with open(filepath, 'w', encoding='utf-8', errors='ignore') as f:
        ...
        save_raw_keyframe_dic(f, raw_keyframe_dic)
...
class BetterExportFbx(Operator, ExportHelper):
    ...
    use_raw_keyframe: BoolProperty(
            name="Raw Keyframe",
            description="Export raw keyframes for objects and FK based meta rig, if you are exporting an IK based advanced rig, such as Rigify Auto-Rigging System, you must set this option disabled, for there are usually no raw keyframes on deform bones of an IK based advanced rig",
            default=False,
            )
    ...
    use_tangents: BoolProperty(
            name="Tangents",
            description="Export tangents, most game engines use tangents data, while other 3d softwares don't need tangents data",
            default=False,
            )

    use_export_materials: BoolProperty(
            name="Export Materials",
            description="Export materials for meshes, if you don't want to export any materials, you can turn off this option",
            default=True,
            )
    ...
    my_interpolation_type: EnumProperty(
            name="Interpolation Type",
            description="Key interpolation type",
            items=(('Constant', "Constant", "Constant key interpolation type"),
                   ('Linear', "Linear", "Linear key interpolation type"),
                   ('Cubic', "Cubic", "Cubic key interpolation type")),
            default='Linear',
            )
    ...
    def draw(self, context):
        ...
        row.enabled = not self.use_separate_collection
        ...
        row = box.row()
        row.prop(self, 'use_copy_texture')
        row.enabled = not self.use_embed_media
        row = box.row()
        row.prop(self, 'my_texture_subdirectory')
        row.enabled = not self.use_embed_media
        ...
        row = box.row()
        row.prop(self, 'use_rigify_root_bone')
        row.enabled = self.use_rigify_armature
        ...
        row = box.row()
        row.prop(self, 'use_only_selected_deform_bones')
        row.enabled = self.use_only_deform_bones
        ...
        row = box.row()
        row.prop(self, 'use_raw_keyframe')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'use_timeline_range')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_animation_offset')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'use_driver_based_shape_key')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_animation_type')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_interpolation_type')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'use_concatenate_all')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'use_independent_animation_stack')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_simplify_algorithm')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_simplify_keyframe_factor')
        row.enabled = self.use_animation and self.my_simplify_algorithm != '0'
        row = box.row()
        row.prop(self, 'use_unroll_filter')
        row.enabled = self.use_animation
        ...
        row = box.row()
        row.prop(self, 'use_vertex_format')
        row.enabled = self.use_vertex_animation
        row = box.row()
        row.prop(self, 'use_vertex_space')
        row.enabled = self.use_vertex_animation
        row = box.row()
        row.prop(self, 'my_vertex_frame_start')
        row.enabled = self.use_vertex_animation
        row = box.row()
        row.prop(self, 'my_vertex_frame_end')
        row.enabled = self.use_vertex_animation
        ...
        box.prop(self, 'use_export_materials')
        ...
        row = box.row()
        row.prop(self, 'my_edge_crease_scale')
        row.enabled = self.use_edge_crease

        box = layout.box()
        box.label(text="Batch Options:")
        row = box.row()
        row.prop(self, 'use_separate_files')
        row.enabled = not self.use_separate_collection
        row = box.row()
        row.prop(self, 'use_separate_collection')
        row.enabled = not self.use_separate_files
        row = box.row()
        row.prop(self, 'use_move_to_origin')
        row.enabled = self.use_separate_collection or self.use_separate_files
    ...
    def execute(self, context):
        ...
        # batch export
        if self.use_separate_collection:
            for lc in context.view_layer.layer_collection.children:
                if lc.visible_get() and len(lc.collection.all_objects) > 0:
                    if self.use_move_to_origin:
                        # find root empty node
                        empty_node = None
                        root_empty_count = 0
                        for ob in lc.collection.objects:
                            if ob.type == 'EMPTY' and ob.parent == None:
                                empty_node = ob
                                root_empty_count += 1
                        root_empty_node = empty_node if root_empty_count == 1 else None
                    context.view_layer.active_layer_collection = lc
                    if self.use_selection:
                        context_objects = list(ob for ob in context.view_layer.active_layer_collection.collection.all_objects if ob.select_get())
                    else:
                        context_objects = list(ob for ob in context.view_layer.active_layer_collection.collection.all_objects)
                    if self.use_visible:
                        context_objects = list(ob for ob in context_objects if ob.visible_get())
                    # remove root empty node
                    if self.use_move_to_origin and root_empty_node in context_objects:
                        context_objects.remove(root_empty_node)
                    save_translations = {}
                    if self.use_move_to_origin:
                        for context_object in context_objects:
                            # save the translation
                            if root_empty_node != None:
                                if context_object.parent == root_empty_node:
                                    save_translations[context_object] = [context_object.matrix_world[0][3], context_object.matrix_world[1][3], context_object.matrix_world[2][3]]
                            else:
                                if context_object.parent == None:
                                    save_translations[context_object] = [context_object.matrix_world[0][3], context_object.matrix_world[1][3], context_object.matrix_world[2][3]]
                            # reset the translation
                            if root_empty_node != None:
                                if context_object.parent == root_empty_node:
                                    context_object.matrix_world[0][3] -= root_empty_node.matrix_world[0][3]
                                    context_object.matrix_world[1][3] -= root_empty_node.matrix_world[1][3]
                                    context_object.matrix_world[2][3] -= root_empty_node.matrix_world[2][3]
                            else:
                                if context_object.parent == None:
