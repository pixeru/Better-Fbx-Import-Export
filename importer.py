    ...
    for (key, material) in material_dic.items():
        ...
        if bpy.context.scene.render.engine == 'CYCLES' or bpy.context.scene.render.engine == 'BLENDER_EEVEE' or bpy.context.scene.render.engine == 'BLENDER_EEVEE_NEXT':
            ...
            if bsdf != None and material_output != None:
                input_name_list = ['Base Color', 'Metallic', 'Specular' if bpy.app.version < (4, 0) else 'Specular IOR Level', 'Roughness', 'Emission' if bpy.app.version < (4, 0) else 'Emission Color', 'Alpha', 'Normal', 'Bump', 'Displacement', 'Ambient', 'Emission Strength']
                ...
                for (i, input_name) in enumerate(input_name_list):
                    ...
                    if i == 0 or i == 4:
                        ...
                        bsdf.inputs[input_name].default_value = material[45]
                    # setup texture image, exclude emission strength
                    if i != 10 and material[i*4+4] != -1:
...
def make_pivot_dic(context, hierarchy_dic, node_dic, pivot_dic):
    for (key, hierarchy) in hierarchy_dic.items():
        if hierarchy[0] == 'MESH':
            mesh_key = key
            ...
            if 'Pivot' in node:
                ob = node['Object']
                pivot = mathutils.Vector(pivot_dic[node['Pivot'][0][1]])
                # apply pivot
                ob.data.transform(mathutils.Matrix.Translation(-pivot))
                ob.matrix_world.translation = ob.matrix_world @ pivot
...
def make_fcurves_list(action_ob, ob, parent_bind_pose, bind_pose, parent_pose_list, pose_list, correct_pose, ob_type, my_rotation_mode, my_animation_offset, selected_bone_ob_bind_pose, selected_bone_ob_parent_bind_pose, imported_bone_ob_bind_pose, imported_bone_ob_parent_bind_pose, use_raw_keyframe, raw_keyframe_map):
    if use_raw_keyframe:
        ...
        for i in range(3):
            channel_location[i] = action_ob.fcurves.new(data_path=ob.path_from_id("location"), index=i, action_group=ob.name)
        if my_rotation_mode == 'QUATERNION':
            for i in range(4):
                channel_rotation[i] = action_ob.fcurves.new(data_path=ob.path_from_id("rotation_quaternion"), index=i, action_group=ob.name)
        elif my_rotation_mode == 'AXIS_ANGLE':
            for i in range(4):
                channel_rotation[i] = action_ob.fcurves.new(data_path=ob.path_from_id("rotation_axis_angle"), index=i, action_group=ob.name)
        else:
            for i in range(3):
                channel_rotation[i] = action_ob.fcurves.new(data_path=ob.path_from_id("rotation_euler"), index=i, action_group=ob.name)
        for i in range(3):
            channel_scale[i] = action_ob.fcurves.new(data_path=ob.path_from_id("scale"), index=i, action_group=ob.name)
        # write to channels
        ...
        for (frame_counter, pose) in enumerate(pose_list):
            ...
            for i in range(3):
                if ('location', i, key_frame + my_animation_offset) in raw_keyframe_map:
                    new_keyframe = channel_location[i].keyframe_points.insert(key_frame + my_animation_offset, loc[i])
                    new_keyframe.interpolation = raw_keyframe_map[('location', i, key_frame + my_animation_offset)]
            if my_rotation_mode == 'QUATERNION':
                for i in range(4):
                    if ('rotation_euler', 0, key_frame + my_animation_offset) in raw_keyframe_map or ('rotation_euler', 1, key_frame + my_animation_offset) in raw_keyframe_map or ('rotation_euler', 2, key_frame + my_animation_offset) in raw_keyframe_map:
                        new_keyframe = channel_rotation[i].keyframe_points.insert(key_frame + my_animation_offset, rot[i])
                        if ('rotation_euler', 0, key_frame + my_animation_offset) in raw_keyframe_map:
                            new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', 0, key_frame + my_animation_offset)]
                        elif ('rotation_euler', 1, key_frame + my_animation_offset) in raw_keyframe_map:
                            new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', 1, key_frame + my_animation_offset)]
                        elif ('rotation_euler', 2, key_frame + my_animation_offset) in raw_keyframe_map:
                            new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', 2, key_frame + my_animation_offset)]
            elif my_rotation_mode == 'AXIS_ANGLE':
                for i in range(4):
                    if ('rotation_euler', 0, key_frame + my_animation_offset) in raw_keyframe_map or ('rotation_euler', 1, key_frame + my_animation_offset) in raw_keyframe_map or ('rotation_euler', 2, key_frame + my_animation_offset) in raw_keyframe_map:
                        new_keyframe = channel_rotation[i].keyframe_points.insert(key_frame + my_animation_offset, rot_axis_angle[i])
                        if ('rotation_euler', 0, key_frame + my_animation_offset) in raw_keyframe_map:
                            new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', 0, key_frame + my_animation_offset)]
                        elif ('rotation_euler', 1, key_frame + my_animation_offset) in raw_keyframe_map:
                            new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', 1, key_frame + my_animation_offset)]
                ...
                for i in range(3):
                    if ('rotation_euler', i, key_frame + my_animation_offset) in raw_keyframe_map:
                        new_keyframe = channel_rotation[i].keyframe_points.insert(key_frame + my_animation_offset, rot_euler[i])
                        new_keyframe.interpolation = raw_keyframe_map[('rotation_euler', i, key_frame + my_animation_offset)]
            for i in range(3):
                if ('scale', i, key_frame + my_animation_offset) in raw_keyframe_map:
                    new_keyframe = channel_scale[i].keyframe_points.insert(key_frame + my_animation_offset, sca[i])
                    new_keyframe.interpolation = raw_keyframe_map[('scale', i, key_frame + my_animation_offset)]
    else:
        key_frames = len(pose_list)
        channel_location = [None] * 3
        channel_rotation = [None] * 4
...
def make_pose_key_dic(context, hierarchy_dic, node_dic, bind_pose_dic, pose_key_dic, my_rotation_mode, my_animation_offset, use_animation_prefix, use_attach_to_selected_armature, selected_armature, selected_armature_bind_pose_dic, imported_armature_bind_pose_dic, obj_name, use_raw_keyframe, raw_keyframe_dic):
    for (key, hierarchy) in hierarchy_dic.items():
        if hierarchy[0] != 'BONE':
            node = node_dic[key]
            parent_key = hierarchy[2]
            ...
            if 'PoseKey' in node:
                raw_keyframe_map = {}
                if use_raw_keyframe:
                    if 'RawKeyframe' in node:
                        raw_keyframe_index = node['RawKeyframe'][0][1]
                        raw_keyframes = raw_keyframe_dic[raw_keyframe_index]
                        for raw_keyframe in raw_keyframes:
                            raw_keyframe_map[(raw_keyframe[0], raw_keyframe[1], raw_keyframe[2] + my_animation_offset)] = raw_keyframe[3]
                action_list = node['PoseKey']
                for action in action_list:
                    ...
                    make_fcurves_list(action_ob, ob, ob_parent_bind_pose, ob_bind_pose, parent_pose_list, pose_list, None, ob.type, my_rotation_mode, my_animation_offset, None, None, None, None, use_raw_keyframe, raw_keyframe_map)
                ...
                if ob.data.shape_keys.animation_data == None:
                    ...
                    for action in action_list:
                        ...
                        for block in ob.data.shape_keys.key_blocks:
                            channel_value.append(action_ob.fcurves.new(data_path=block.path_from_id("value"), action_group=ob.name))
            ...
            if 'CameraKey' in node:
                ...
                if ob.data.animation_data == None:
                    ...
                    for action in action_list:
                        ...
                        channel_value = []
                        fcurve = action_ob.fcurves.new(data_path=ob.data.path_from_id("lens"), action_group=ob.name)
                        fcurve.keyframe_points.add(len(camera_key_dic[action_index]))
                        fcurve2 = action_ob.fcurves.new(data_path=ob.data.dof.path_from_id("focus_distance"), action_group=ob.name)
...
def parse_pivot_dic(pivot_dic, tokens):
    pivot_dic[int(tokens[1])] = [float(tokens[2]), float(tokens[3]), float(tokens[4])]
...
def parse_material_dic(material_dic, tokens):
    ...
    material_dic[int(tokens[1])].append(float(tokens[47]))
...
def read_some_data(context, filepath, my_leaf_bone, my_import_normal, my_shade_mode, use_auto_smooth, my_angle, use_auto_bone_orientation, my_bone_length, my_calculate_roll, use_vertex_animation, use_edge_crease, my_edge_crease_scale, my_edge_smoothing, use_import_materials, obj_name, my_rotation_mode, use_detect_deform_bone, use_fix_bone_poses, use_attach_to_selected_armature, my_animation_offset, use_animation_prefix, primary_bone_axis, secondary_bone_axis, use_pivot, use_raw_keyframe):
        ...
        for line in f:
            ...
            if line.startswith("[") and line.endswith("]"):
                ...
                if len(tokens) > 0:
                    ...
                    # parse pivot dictionary
                    elif section == "Pivot":
                        # validate data length
                        if len(tokens) == 5:
                            parse_pivot_dic(pivot_dic, tokens)
                    ...
                    # parse raw keyframe dictionary
                    elif section == "RawKeyframe":
                        # validate data length
                        if len(tokens) == 8:
                            parse_raw_keyframe_dic(raw_keyframe_dic, tokens)
    ...
    # make pivot dic
    if use_pivot:
        make_pivot_dic(context, hierarchy_dic, node_dic, pivot_dic)
    # make shape dic
...
class BetterImportFbx(Operator, ImportHelper):
    ...
    use_raw_keyframe: BoolProperty(
            name="Raw Keyframe",
            description="Import raw keyframes for objects and FK based meta rig, if the animation in the FBX file is already baked keyframes, please set this option disabled",
            default=False,
            )
    ...
    use_separate_collection: BoolProperty(
            ...
            description="Import files into separate collections which collection names are the filenames",
            default=False,
            )
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Shading Options:")
        box.prop(self, 'use_auto_smooth')
        box.prop(self, 'my_shade_mode')
        row = box.row()
        row.prop(self, 'my_angle')
        row.enabled = self.use_auto_smooth or self.my_shade_mode == 'Smooth'
        ...
        row = box.row()
        row.prop(self, 'use_attach_to_selected_armature')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'my_animation_offset')
        row.enabled = self.use_animation
        row = box.row()
        row.prop(self, 'use_animation_prefix')
        row.enabled = self.use_animation
        ...
        box.prop(self, 'use_separate_collection')
